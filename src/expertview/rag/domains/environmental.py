"""Environmental-domain RAG loader.

Reads `data/domains/environmental/*.md`, wraps each file in the project's
pydantic `Document`, and returns an `InMemoryKnowledgeStore` populated with
the corpus. Embeddings are passed in by the wiring layer (the runner) - this
module does not construct provider clients per the `CLAUDE.md` architecture
rule.

A pickle cache under `.cache/rag/environmental/` skips re-embedding when the
corpus manifest and embedding signature both match. Cache invalidation is
automatic on either corpus change or embedding-model swap.
"""
...
CORPUS_DIR = Path("data/domains/environmental")


def load_environmental_store(embeddings: Embeddings) -> InMemoryKnowledgeStore:
    paths = _corpus_paths()
