"""Process-domain RAG loader.

Reads `data/domains/process/*.md`, wraps each file in the project's
...
A pickle cache under `.cache/rag/process/` skips re-embedding when the corpus
manifest and embedding signature both match. Cache invalidation is automatic
on either corpus change or embedding-model swap.
"""
...
DOMAIN = "process"
CORPUS_DIR = Path("data/domains/process")


def load_process_store(embeddings: Embeddings) -> InMemoryKnowledgeStore:
    paths = _corpus_paths()
