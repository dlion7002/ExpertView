"""Process investigator LangGraph node factory."""
...
__all__ = ["make_process_investigator_node"]

_LOGGER = get_logger("expertview.investigator")
_DOMAIN = "process"

_PROMPT_PATH = Path(__file__).parents[2] / "prompts" / "investigator" / "process.md"
...
ProcessInvestigatorNode = Callable[[ExpertViewState], Awaitable[dict[str, list[Finding]]]]
...
def make_process_investigator_node(
    store: KnowledgeStore,
    llm: InvestigatorLlm,
    *,
    k: int = 5,
) -> ProcessInvestigatorNode:
    prompt_template = Template(_PROMPT_PATH.read_text(encoding="utf-8"))

    async def process_investigator_node(
    ...
    return process_investigator_node
...
def _response_text(response: object) -> str:
    ...
    if isinstance(content, str):
        return content
    raise TypeError("Process investigator LLM response content must be text.")
...
def _validate_findings(findings: list[Finding], documents: list[Document]) -> None:
    ...
    for finding in findings:
        if finding.investigator_domain != "process":
            raise ValueError('Process investigator findings must use domain "process".')
        if not finding.citations:
            raise ValueError("Process investigator findings must include at least one citation.")
        unknown = sorted(set(finding.citations) - valid_citations)
        if unknown:
            raise ValueError(
                "Process investigator citations must reference retrieved document "
