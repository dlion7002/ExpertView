"""Environmental investigator LangGraph node factory."""
...
__all__ = ["make_environmental_investigator_node"]

_PROMPT_PATH = Path(__file__).parents[2] / "prompts" / "investigator" / "environmental.md"
...
EnvironmentalInvestigatorNode = Callable[[ExpertViewState], Awaitable[dict[str, list[Finding]]]]
...
def make_environmental_investigator_node(
    store: KnowledgeStore,
    llm: InvestigatorLlm,
    *,
    k: int = 5,
) -> EnvironmentalInvestigatorNode:
    prompt_template = Template(_PROMPT_PATH.read_text(encoding="utf-8"))

    async def environmental_investigator_node(
    ...
    return environmental_investigator_node
...
def _response_text(response: object) -> str:
    ...
    if isinstance(content, str):
        return content
    raise TypeError("Environmental investigator LLM response content must be text.")
...
def _validate_findings(findings: list[Finding], documents: list[Document]) -> None:
    ...
    for finding in findings:
        if finding.investigator_domain != "environmental":
            raise ValueError('Environmental investigator findings must use domain "environmental".')
        if not finding.citations:
            raise ValueError(
                "Environmental investigator findings must include at least one citation."
            )
        unknown = sorted(set(finding.citations) - valid_citations)
        if unknown:
            raise ValueError(
                "Environmental investigator citations must reference retrieved document "
