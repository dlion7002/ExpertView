...
__all__ = ["make_mechanical_investigator_node"]
...
def make_mechanical_investigator_node(
    store: KnowledgeStore,
    llm: InvestigatorLlm,
    *,
    k: int = 5,
) -> MechanicalInvestigatorNode:
    prompt_template = Template(_PROMPT_PATH.read_text(encoding="utf-8"))

    async def mechanical_investigator_node(
    ...
    return mechanical_investigator_node
...
def _response_text(response: object) -> str:
    ...
    if isinstance(content, str):
        return content
    raise TypeError("Mechanical investigator LLM response content must be text.")
...
def _validate_findings(findings: list[Finding], documents: list[Document]) -> None:
    ...
    for finding in findings:
        if finding.investigator_domain != "mechanical":
            raise ValueError('Mechanical investigator findings must use domain "mechanical".')
        if not finding.citations:
            raise ValueError("Mechanical investigator findings must include at least one citation.")
        unknown = sorted(set(finding.citations) - valid_citations)
        if unknown:
            raise ValueError(
                "Mechanical investigator citations must reference retrieved document "
