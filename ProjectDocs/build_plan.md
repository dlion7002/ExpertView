...
- **Outputs**:
  - `pyproject.toml` (uv-managed) with `langgraph`, `langchain-core`, `langchain-anthropic`, `langchain-nvidia-ai-endpoints`, `langchain-community`, `langsmith`, `pydantic`, `structlog`, `rich`, `pytest`, `pytest-asyncio`, `ruff`. `.gitignore`, `.python-version`.
  - `.env.example` listing `NVIDIA_API_KEY`, `ANTHROPIC_API_KEY`, `LANGSMITH_API_KEY`, `LANGSMITH_PROJECT`, `EXPERTVIEW_SYNTH_MODEL`.
  ...
  - `agents/llms.py` with `ChatAnthropic` + `ChatNVIDIA` factory; honors `EXPERTVIEW_SYNTH_MODEL`.
  - `tests/unit/test_schemas.py` — round-trip JSON serialization for every pydantic model.
  - `tests/integration/test_provider_keys.py` — single-call round-trip against NV-Embed-v2, Llama 3.3 70B (one-token completion), and (optionally, gated by env) DeepSeek-R1 / Opus 4.7. Verifies keys + connectivity.
...
- **Outputs**:
  ...
  - `rag/domains/mechanical.py` — loader: reads `data/domains/mechanical/*.md`, embeds via `NVIDIAEmbeddings(model="nvidia/nv-embed-v2")`, caches the vector index to a local file so we re-embed only on corpus change.
  - `agents/investigators/mechanical.py` — async LangGraph node: queries the `KnowledgeStore`, optionally reranks via `NVIDIARerank`, calls `ChatNVIDIA(model="meta/llama-3.3-70b-instruct")`, returns `{"findings": [...]}`.
  - `agents/synthesizer.py` — terminal LangGraph node: calls the LLM selected by `agents/llms.py` (`EXPERTVIEW_SYNTH_MODEL=deepseek-r1` during build), returns `{"causal_report": ...}`.
...
- **Risk**: the prompt produces low-quality findings on DeepSeek-R1's reasoning style. Mitigation: iterate on the prompt template against DeepSeek-R1; demand citations; demand structured `Finding` output via pydantic-validated JSON; rehearse the same prompt against Opus 4.7 at the end of phase 5 to catch transfer drift.
...
- **Risk**: NIM free-tier rate limits during parallel dispatch. Mitigation: throttle via an `asyncio.Semaphore` inside the dispatcher (cap of 5 is fine; lower if NIM 429s); on-429 retry with backoff inside `agents/llms.py`. Llama 3.3 70B is the parallel-friendly choice exactly to keep this risk manageable.
...
- **Goal**: one fully scripted scenario, retries on transient LLM / NIM failures, deterministic mock data for reproducible demos, the synthesizer swap to Opus 4.7 exercised on the demo laptop, a LangSmith trace replay path as the final fallback.
- **Outputs**:
  - Retry/backoff wrapper inside `agents/llms.py` covering both `ChatNVIDIA` and `ChatAnthropic` (429s, transient 5xxs).
  - Deterministic seed for any sampling; embedding cache invalidation rule documented.
  - **Synthesizer swap rehearsal**: a full demo run with `EXPERTVIEW_SYNTH_MODEL=opus-4-7` on the demo laptop, verifying the same prompts produce a coherent CausalReport. This is where prompt-transfer drift between DeepSeek-R1 and Opus 4.7 is caught.
  ...
  - `README.md` with a 1-minute run-it-yourself guide (lists the three env vars: `NVIDIA_API_KEY`, `ANTHROPIC_API_KEY`, `LANGSMITH_API_KEY`).
- **Quality gate**: three consecutive clean runs of the demo scenario on the actual demo laptop — at least one of them with `EXPERTVIEW_SYNTH_MODEL=opus-4-7` to confirm the demo-path. A successful LangSmith trace is exported.
...
2. `src/expertview/agents/llms.py` — the `ChatAnthropic` + `ChatNVIDIA` factory + env-driven model swap. Reusable verbatim.
...
| NIM monthly free-tier quota exhausted by iteration | LangSmith trace replay during prompt iteration; track NIM usage in LangSmith metadata. Only phase-7 rehearsals hit fresh API calls. |
...
| Anthropic spend during prompt iteration | Iteration runs `EXPERTVIEW_SYNTH_MODEL=deepseek-r1` (free); Opus 4.7 only at phase-7 rehearsal and demo time. |
| Prompt-transfer drift between DeepSeek-R1 and Opus 4.7 | Phase-7 explicitly rehearses with the Opus 4.7 swap on the demo laptop. |
...
| NIM rate-limit during live demo | Investigator dispatcher already throttles via `Semaphore`; on-429 retry inside `agents/llms.py`. |
| Anthropic spend overrun at the event | The synthesizer is the only Anthropic call. Budget caps a single demo run cost. Mid-event iteration can be done with DeepSeek-R1 to preserve budget; switch to Opus 4.7 only when demoing. |
...
4. **Show the convergence**: the synthesizer (Opus 4.7) produces the CausalReport with weighted hypotheses and citations.
5. **Close** with the LangGraph + architecture diagrams, the multi-provider model split story (Llama 3.3 70B for parallelism, DeepSeek-R1 / Opus 4.7 for synthesis, NV-Embed-v2 + NV-Rerank for retrieval, LangSmith for traceability), and the reusability claim (Path B lift-ability).
